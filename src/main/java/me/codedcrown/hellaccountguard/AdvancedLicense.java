...
public class AdvancedLicense {
    ...
    public boolean register() {
        log(0, "[]==========[HellHost-LicenseSystem]==========[]");
        log(0, "Вход в Лиценз-Сървъра...");
        ValidationType vt = isValid();
        if (vt == ValidationType.VALID) {
            log(1, "Лиценза е валиден!");
            log(0, "[]==========[HellHost-LicenseSystem]==========[]");
            return true;
        } else {
            log(1, "Лиценза не е валиден!");
            log(1, "Неуспешно в резултат на " + vt.toString());
            log(1, "Приставката е изключена!");
            log(0, "[]==========[HellHost-LicenseSystem]==========[]");
